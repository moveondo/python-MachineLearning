...
def testRbf(k1=1.3):
    dataArr, labelArr = loadDataSet('./db/testSetRBF.txt')
    ...
    dataArr, labelArr = loadDataSet('./db/testSetRBF2.txt')
    errorCount = 0
    datMat = mat(dataArr)
    labelMat = mat(labelArr).transpose()
    m, n = shape(datMat)
...
def testDigits(kTup=('rbf', 10)):
    ...
    # 1. 导入训练数据
    dataArr, labelArr = loadImages('./db/trainingDigits')
    ...
    # 2. 导入测试数据
    dataArr, labelArr = loadImages('./db/testDigits')
    errorCount = 0
    datMat = mat(dataArr)
    labelMat = mat(labelArr).transpose()
    m, n = shape(datMat)
...
if __name__ == "__main__":
    ...
    # 无核函数的测试
    # 获取特征和目标变量
    dataArr, labelArr = loadDataSet('./db/testSet.txt')
    # print labelArr
